...
var storedTotalCumulativeShares = 0;

var poolCapacity = {};
var poolCapacityTotal = 0;
var poolCapacityAccounts = {};
var lastCapacityCleaning = 1;
function CleanCapacity(height) {
    if (height - lastCapacityCleaning > 0) {
 	console.log("Cleaning pool capacity");
        lastCapacityCleaning = height;
        poolCapacityTotal = 0;
        for (var accountId in poolCapacity) {
            poolCapacityAccounts[accountId] = 0;
            for (var ip in poolCapacity[accountId]) {
                ipdata = poolCapacity[accountId][ip];
                if (ipdata.height >= height - 8) {
                    cap = parseFloat(ipdata.capacity);
                    poolCapacityAccounts[accountId] += cap;
                    poolCapacityTotal += cap;
                } else {
                    delete poolCapacity[accountId][ip];
                }

            }
            if (poolCapacityAccounts[accountId] == 0) {
                delete poolCapacity[accountId];
                delete poolCapacityAccounts[accountId];
            }
...
function AddCapacity(accountId, ip, capacity, height) {
    if (typeof accountId == 'undefined' || typeof ip == 'undefined' || typeof capacity == 'undefined') return;
    if (!poolCapacity.hasOwnProperty(accountId)) {
        poolCapacity[accountId] = {};
    }
    if (!poolCapacity[accountId].hasOwnProperty(ip)) {
        poolCapacity[accountId][ip] = {};
    }
    poolCapacity[accountId][ip].capacity = capacity;
    poolCapacity[accountId][ip].height = height;
}
...
AccountShare.prototype.deleteRoundShareByDistance = function (distance) {
    CleanCapacity(this.currentRoundShare.height);
...
PoolShare.prototype.getTotalCumulativeShares = function () {

    return storedTotalCumulativeShares;
...
PoolShare.prototype.getCumulativeShares = function () {
    ...
    for (var accountId in blockShareList) {
        ...
        if (poolCapacityAccounts.hasOwnProperty(accountId)) cumulativeShare[accountId].capacity = parseFloat(poolCapacityAccounts[accountId] / 1024).toFixed(2);
        else cumulativeShare[accountId].capacity = 0;
    }
    storedTotalCumulativeShares = total;
...
module.exports = {
    ...
    getTotalCumulativeShares: function () {
        return poolShare.getTotalCumulativeShares();
    },
    getCumulativeShares: function () {
        return poolShare.getCumulativeShares();
    },
    getPoolCapacity: function () {
        return poolCapacityTotal;
    },
